...
class _MyHomePageState extends State<MyHomePage> {
  ...
  Future<void> _initShop() async {
    var rsaKey = " ";
    try {
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              FilledButton(onPressed: checkUserPurchasedItem, child: Text('Check if user purchased this item')),
              FilledButton(onPressed: checkUserSubscribedItem, child: Text('Check if user subscribed this item')),
              FilledButton(
                  onPressed: () {
              ...
              FilledButton(onPressed: checkTrialSubscription, child: Text('Check Trial subscription'))
            ],
          ),
        ),
      ),
  ...
  Future<void> subscribeProduct(
    ...
    try {
      PurchaseInfo? response = await FlutterPoolakey.subscribe(productId, payload: payload, dynamicPriceToken: dynamicPriceToken ?? "");
    } catch (e) {
  ...
  Future<void> purchaseProduct(
    ...
    try {
      PurchaseInfo? response = await FlutterPoolakey.purchase(productId, payload: payload, dynamicPriceToken: dynamicPriceToken ?? "");
      if (consume) {
  ...
  Future<void> checkUserSubscribedItem() async {
    ...
    try {
      List<PurchaseInfo>? response = await FlutterPoolakey.getAllSubscribedProducts();
      bool result = response.any((element) => element.productId == productIdController.text);
      if (result) {
  ...
  Future<void> checkUserPurchasedItem() async {
    ...
    try {
      List<PurchaseInfo>? response = await FlutterPoolakey.getAllPurchasedProducts();
      bool result = response.any((element) => element.productId == productIdController.text);
      if (result) {
        showSnackBar("User has bought this item");
      } else {
  ...
  Future<void> getSkuDetailOfSubscriptionItem(String skuValueInput) async {
    ...
    try {
      List<SkuDetails>? response = await FlutterPoolakey.getSubscriptionSkuDetails([skuValueInput]);
  ...
  Future<void> getSkuDetailOfInAppItem(String skuValueInput) async {
    ...
    try {
      List<SkuDetails>? response = await FlutterPoolakey.getInAppSkuDetails([skuValueInput]);
